  ...
  <nav class="glass-nav sticky top-0 z-40 px-6 py-4">
    <div class="max-w-7xl mx-auto flex items-center justify-between">
      ...
      <div class="flex items-center gap-3">
                                ...
                                <!-- BACKEND API SERVER CONNECT BUTTON -->
        <button type="button" id="btn-backend-status" onclick="openBackendModal()" class="px-3.5 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 flex items-center gap-2 text-xs font-bold transition-all shadow">
          <span id="backend-status-dot" class="w-2.5 h-2.5 rounded-full bg-amber-400 animate-pulse"></span>
          <span id="backend-status-text" class="hidden sm:inline">Backend API</span>
        </button>
...
<!-- AUTH MODAL -->
  ...
  <script>
    ...
    function getApiBaseUrl() {
      ...
      // Default to Local Backend (or Cloud fallback) for GitHub Pages
    ...
    async function checkBackendConnection() {
      ...
      const currentUrl = getApiBaseUrl() || 'http://127.0.0.1:8000';

      try {
        const res = await fetch(currentUrl + '/health', { method: 'GET' });
        if (res.ok) {
          if (dot) {
      ...
      if (text) text.innerText = 'Backend: Offline (Click)';
    }
